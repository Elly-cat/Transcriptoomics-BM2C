...
msig_go_bp <- msigdbr(species = "Homo sapiens", collection = "C5", subcollection = "BP") #vervang BP naar CC of MF voor andere processen
...
MF <- ggplot(top_terms, aes(x = reorder(gs_name, -Direction_num * log10(PValue)),
  ...
  theme_minimal() + scale_fill_manual(values = c("steelblue2")) #vernader de kleur van de proccesen naar wat je wil
MF #zie plot

#maak de combo plot
plot <- (BP / CC / MF) + 
  plot_annotation(title = "3 processen pathways voor reuma geraleerde DE die opgereguleerd zijn") &
...
ggsave("Rplot_3_paths_Up.png", plot = plot, width = 12, height = 8, dpi = 300)

#---------------------------down regulated plot---------------------------------------

# filter alleen 'Downregulated' pathways
camera_down <- camera_sig %>% 
  filter(Direction == "Down")

# selecteer de top 10 op basis van p-waarde
top_terms <- head(camera_down[order(camera_down$PValue), ], 10)

#Voeg rownames als kolom toe
top_terms$gs_name <- rownames(top_terms)

#Toevoegen numerieke waarde voor richting (allemaal 1 in dit geval)
top_terms$Direction_num <- 1  # alles is Down

#Plot alleen downregulated pathways (bijv. GO-MF)
MFD <- ggplot(top_terms, aes(
  ...
  labs(
    x = "Genenset (GO term)",
    y = "-log10(p-waarde)",
    title = "GO-MF (Downregulated)") +
  theme_minimal() +
  scale_fill_manual(values = c("Down" = "steelblue2"))  # lichtblauw voor Down

#Laat de plot zien
MFD

#combo plot voor downregulated
plot2 <- (BPD / CCD / MFD) + 
  plot_annotation(title = "3 processen pathways voor reuma geraleerte DE die neergeruguleerd zijn") &
  theme(plot.title = element_text(hjust = 0.5), axis.text.y = element_text(size = 6.3))
...
#sla de plot op
ggsave("Rplot_3_paths_down2.png", plot = plot2, width = 12, height = 8, dpi = 300)
